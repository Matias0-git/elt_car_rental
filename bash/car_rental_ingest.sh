...
for i in "${!URLS[@]}"; do
  ...
  # -O (mayúscula) es para el nombre de archivo de salida
  wget -O "$LOCAL_FILE_PATH" "$FILE_URL"
  # set -e se encargará de detener el script si wget falla

  # 2b. Enviar el archivo a HDFS usando la ruta absoluta
  echo "Enviando $LOCAL_FILE_PATH a HDFS en $HDFS_INGEST_DIR..."
  # Usamos la variable de la ruta local completa
  $HDFS_CMD dfs -put -f "$LOCAL_FILE_PATH" "$HDFS_INGEST_DIR"
  # set -e se encargará de detener el script si hdfs dfs -put falla
